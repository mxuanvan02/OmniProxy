...
// group that became empty. svgEntryFileName sanitises the ids onto a safe stem,
// so the joined path cannot escape the group directory whatever the caller
// passes. A missing file is not an error here: removed=false lets the handler
// answer 404 without distinguishing "never stored" from "already gone".
